...
//用于socket client测试, add by cursorhu
#ifdef TEST_SOCK
void test_sock(int);
#else
...
void accept_request(void *arg)
{
 ...
 else
 {
  if ((st.st_mode & S_IFMT) == S_IFDIR)
   strcat(path, "/index.html");
  //如果你的文件默认是有执行权限的，自动解析成cgi程序，如果有执行权限但是不能执行，会收到到报错信号
  if ((st.st_mode & S_IXUSR) ||
      (st.st_mode & S_IXGRP) ||
      (st.st_mode & S_IXOTH)    )
   cgi = 1;
  if (!cgi)
   //(对于GET请求)返回.html文件给http客户端
...
#ifdef TEST_SOCK
void test_sock(int client_sock)
{
  char buf;
  int size;

  size = read(client_sock, &buf, sizeof(buf)); //read是阻塞式
  if(size == 0)
      return;
  
  write(client_sock, &buf, sizeof(buf));
  close(client_sock); //这里close, client才能结束read阻塞
}
#endif
/**********************************************************************/
...
int main(void)
{
 ...
 server_sock = startup(&port); //初始化server socket, sock-bind-listen三连
